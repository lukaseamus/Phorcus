...
pp <- ggplot(data = new) +
        ...
        annotate("text", x = c(-5.1, -5.55, -5.8), y = c(0.062, 0.062, 0.065), size = 4.2, hjust = 0, parse = T,
                 label = c("italic(N)*' = 100'", "bold(Equation)", "y == tan^-1*(-1.9*x-0.14)/pi+0.5")) +
        labs(y = expression("Predation risk (d"^-1*")"),
             x = "Position in relation to lowest astronomical tide (m)") +
...
dp <- ggplot() +
        ...
        theme(legend.position = c(0.2, 0.045)) +
        mytheme +
