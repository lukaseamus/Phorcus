...
d$site <- factor(d$site, levels = c("Ras", "Dwejra", "Xwejni"))

require(ggridges)
plot <- ggplot(data = d) +
  geom_density_ridges(aes(adjusted, site),
                      scale = 1.24, bandwidth = 8)
...
denp <- ggplot() +
        geom_density_ridges_gradient(data = d[d$site == "Ras",],
                                     ...
                                     scale = 49, bandwidth = 8, calc_ecdf = TRUE, colour = "#a1d5cf",
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 1,
        ...
        annotate("text", x = 500, y = 1.15:3.15, size = 4.2, hjust = 0, angle = 270,
                 family = "Helvetica Neue", label = c("Id-Dejjaq tar-Ras",
                                                      "Il-Qala tad-Dwejra",
                                                      "Il-Bajja tax-Xwejni")) +
        ...
        scale_y_discrete(position = "right", expand = expansion(add = c(0, 1.1)),
                         labels = c(expression(italic("n ")*"= 100"),
                                    ...
                                    expression(italic("n ")*"= 100"))) +
        coord_flip(xlim = c(0, 500), clip = "off") +
        mytheme +
...
dis$site <- factor(dis$site, levels = c("Ras", "Dwejra", "Xwejni"))
...
disp <- ggplot() +
        ...
        scale_y_discrete(position = "right", expand = expansion(add = c(0, 1.2)),
                         labels = c(expression(italic("n ")*"= 210"),
                                    expression(italic("n ")*"= 293"),
                                    expression(italic("n ")*"= 217"))) +
        coord_flip(xlim = c(0, 0.8), clip = "off") +
        mytheme +
        theme(axis.ticks.x = element_blank(),
              axis.line.x = element_blank(),
              axis.text.x = element_text(hjust = 0),
...
#### 3.3 Combined plot ####
require(cowplot)
finalp <- plot_grid(denp, disp, labels = c("b", "c"), label_size = 15, align = "v",
                    label_fontfamily = "Helvetica Neue", nrow = 2)
finalp # 4 x 6 in
