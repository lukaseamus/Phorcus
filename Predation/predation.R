...
pp <- ggplot(data = new) +
        ...
        annotate("text", x = c(-4.8, -5.25), y = c(0.066, 0.063), size = 4.2, hjust = 0, parse = T,
                 label = c("italic('N ')*'= 100'", "bold(Equation)"), family = "Helvetica Neue") +
        geom_text(aes(-5.7, 0.066), 
                  label = expression(italic("y ")*"= "*frac("tan"^-1*"(–1.9"*italic("x ")*"– 0.14)", pi)*" + 0.5"),
...
dp <- ggplot() +
        ...
        geom_density_ridges_gradient(data = d[d$species == "Hexaplex trunculus",],
                                     ...
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 3,
        ...
        geom_density_ridges_gradient(data = d[d$species == "Hermodice carunculata",],
                                     ...
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 2.14,
                                     quantile_lines = TRUE, quantiles = c(0.025, 0.5, 0.975)) +
        ...
        geom_density_ridges_gradient(data = d[d$species == "Thalassoma pavo",],
                                     ...
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 0.28,
                                     quantile_lines = TRUE, quantiles = c(0.025, 0.5, 0.975)) +
        ...
        geom_density_ridges_gradient(data = d[d$species == "Stramonita haemastoma",],
                                     ...
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 0.68,
                                     quantile_lines = TRUE, quantiles = c(0.025, 0.5, 0.975)) +
        ...
        geom_density_ridges_gradient(data = d[d$species == "Phorcus turbinatus",],
                                     ...
                                     jittered_points = TRUE, point_alpha = 0.4, point_shape = 16, point_size = 0.21,
                                     quantile_lines = TRUE, quantiles = c(0.025, 0.5, 0.975)) +
...
#### 3.3 Combined plot ####
require(cowplot)
finalp <- plot_grid(pp, dp, labels = "auto", label_size = 15, label_fontfamily = "Helvetica Neue")
finalp # 6 x 8 in
