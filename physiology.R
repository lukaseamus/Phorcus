...
pp <- ggplot(data = new) +
        ...
        annotate("text", x = c(0.152, 0.15, 0.175), y = c(0.21, 0.105, 0.05), size = 4.2, hjust = 0, parse = T,
                 label = c("italic(N)*' = 200'", "bold(Equation)", "y == phi*(-0.76*x+3)")) +
        ...
        mytheme +
        theme(legend.position = c(0.124, 0.305),
...
n <- aggregate(l.ml ~ sp, length, data = l.m)
colnames(n)[2] <- "n"
